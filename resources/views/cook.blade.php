...
<main>
    <div class="container-fluid px-4">
        <h1 class="mt-4">Cooker Bill</h1>
        <ol class="breadcrumb mb-4">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Dashboard</a></li>
            <li class="breadcrumb-item active">Cooker Bill</li>
        </ol>
        <div class="card mb-4">
    ...
    $(document).ready(function() {
        $('.js-example-basic-single').select2({
            ...
            $("#add_btn").click(function(e){
                e.preventDefault();
                $('#submit_btn').text("Add Cooker Bill");
                $('#id').val('');
                $('#data_form').trigger("reset");
                $('.modal-title').html("Add Cooker Bill");
            ...
            $('body').on('click', '.edit', function () {
                var id = $(this).attr('id');
                $.get("{{ route('cooker.index') }}" +'/' + id +'/edit', function (data) {
                $('.modal-title').html("Edit Cooker Bill");
                $('#submit_btn').val("Update Cooker Bill");
                $('#id').val(data.id);
                ...
                $.ajax({
                    ...
                    success: function (response) {            
                        if (response.status == 200) {
                            ...
                            title: 'Success',
                            subtitle: 'Cooker',
                            content: 'Cooker Bill Saved Succesfully',
                            delay: 5000,                            
                            });
                            fetchData();
                            reset();
